...
def convert_openapi_to_mcp_tools(
    ...
    for path, path_item in resolved_openapi_schema.get("paths", {}).items():
        for method, operation in path_item.items():
            ...
            # Organize parameters by type first (needed for summary)
            path_params = []
            query_params = []
            header_params = []
            body_params = []
            ...
            total_params = len(path_params) + len(query_params) + len(body_params)
            optional_count = total_params - len(required_props)

            # Build tool description with structured sections:
            # 1. Summary (concise overview at the top)
            tool_description = generate_tool_summary(
                summary=summary,
            ...
            # 2. Parameters section (required before optional)
            param_docs = format_all_parameters_docs(
                path_params=path_params,
                query_params=query_params,
                body_params=body_params,
                required_props=required_props,
            )
            tool_description += param_docs

            # 3. Responses section (after parameters)
