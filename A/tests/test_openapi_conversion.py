...
def test_generate_param_description_common_names():
    """Test that generate_param_description returns sensible descriptions for common parameter names."""
    # ID patterns
    assert "identifier" in generate_param_description("id", "integer").lower()
    assert "identifier" in generate_param_description("user_id", "string").lower()
    assert "user" in generate_param_description("user_id", "string").lower()
    assert "identifier" in generate_param_description("item_id", "integer").lower()

    # Pagination
    assert "page" in generate_param_description("page", "integer").lower()
    assert "maximum" in generate_param_description("limit", "integer").lower()
    assert "skip" in generate_param_description("offset", "integer").lower()

    # Search/filtering
    assert "search" in generate_param_description("q", "string").lower()
    assert "search" in generate_param_description("search", "string").lower()
    assert "sort" in generate_param_description("sort_by", "string").lower()

    # Common fields
    assert generate_param_description("name", "string") != ""
    assert generate_param_description("email", "string") != ""
    assert generate_param_description("description", "string") != ""
    assert generate_param_description("status", "string") != ""
    assert generate_param_description("price", "number") != ""


def test_generate_param_description_path_params():
    """Test that path parameters get contextual descriptions."""
    # Path parameters should get a description even for unknown names
    desc = generate_param_description("foo_bar", "string", "path")
    assert desc != ""
    assert "foo bar" in desc.lower()


def test_generate_param_description_unknown_names():
    """Test that unknown parameter names return empty string (unless path param)."""
    # Unknown query/body params should return empty
    assert generate_param_description("xyz_unknown", "string", "query") == ""
    assert generate_param_description("custom_field", "integer", "body") == ""

    # But path params always get a description
    assert generate_param_description("xyz_unknown", "string", "path") != ""


def test_params_without_description_get_defaults():
    """Test that parameters without descriptions get auto-generated defaults."""
    # Parameters without descriptions
    path_params = [
        ("user_id", {"name": "user_id", "schema": {"type": "string"}}),
    ]
    query_params = [
        ("page", {"name": "page", "schema": {"type": "integer", "default": 1}}),
        ("limit", {"name": "limit", "schema": {"type": "integer", "default": 10}}),
    ]

    result = format_all_parameters_docs(path_params, query_params, [], ["user_id"])

    # Check that auto-generated descriptions are included
    assert "identifier" in result.lower() or "user" in result.lower()
    assert "page" in result.lower()
    assert "maximum" in result.lower() or "items" in result.lower()


def test_tool_description_consistent_format(simple_fastapi_app: FastAPI):
    """Test that all tool descriptions follow a consistent format structure."""
    openapi_schema = get_openapi(
        title=simple_fastapi_app.title,
        version=simple_fastapi_app.version,
        openapi_version=simple_fastapi_app.openapi_version,
    ...
    for tool in tools:
        ...
        # Every description should start with bold summary
        assert description.startswith("**"), f"Tool {tool.name} should start with bold summary"

        # If it has parameters, should have Parameters section
        if tool.inputSchema.get("properties"):
            assert "### Parameters:" in description, f"Tool {tool.name} should have Parameters section"

        # Should have Responses section
        assert "### Responses:" in description, f"Tool {tool.name} should have Responses section"

        # Section order should be consistent: Summary -> Parameters -> Responses
        params_pos = description.find("### Parameters:")
        responses_pos = description.find("### Responses:")

        if params_pos != -1:
            # Summary (bold text) should come before Parameters
            first_bold_end = description.find("**", 2)  # Find end of first bold
            assert first_bold_end < params_pos, f"Tool {tool.name}: Summary should come before Parameters"

            # Parameters should come before Responses
            assert params_pos < responses_pos, f"Tool {tool.name}: Parameters should come before Responses"


def test_tool_description_format_various_endpoints(complex_fastapi_app: FastAPI):
    """Test description format for various endpoint configurations."""
    openapi_schema = get_openapi(
        title=complex_fastapi_app.title,
        version=complex_fastapi_app.version,
        openapi_version=complex_fastapi_app.openapi_version,
    ...
    # Check list_products (GET with many query params)
    list_products = next(t for t in tools if t.name == "list_products")
    desc = list_products.description or ""
    assert "**Query Parameters:**" in desc
    assert "**Path Parameters:**" not in desc  # No path params
    assert "**Body Parameters:**" not in desc  # GET doesn't have body

    # Check get_product (GET with path param)
    get_product = next(t for t in tools if t.name == "get_product")
    desc = get_product.description or ""
    assert "**Path Parameters:**" in desc
    assert "*required*" in desc  # Path param should be required

    # Check create_order (POST with body params)
    create_order = next(t for t in tools if t.name == "create_order")
    desc = create_order.description or ""
    assert "**Body Parameters:**" in desc
    assert "*required*" in desc  # Should have required body params


def test_contextual_example_generation():
    """Test that example generation produces contextual values for various property names."""
    ...
    # Verify contextual values (not generic placeholders like "string" or 1)
    ...
    assert "2024" in result["created_at"]  # Should be a date string


def test_example_generation_with_enums():
    """Test that example generation uses enum values when available."""
    schema = {
        "type": "string",
    ...
    result = generate_example_from_schema(schema)
    assert result == "pending"  # Should use first enum value


def test_example_generation_formats():
    """Test that example generation handles various string formats."""
    test_cases = [
        ({"type": "string", "format": "uuid"}, "550e8400"),  # Contains UUID-like string
        ({"type": "string", "format": "email"}, "@"),  # Contains @
        ({"type": "string", "format": "uri"}, "https://"),  # Contains URL prefix
        ({"type": "string", "format": "date"}, "2024"),  # Contains year
        ({"type": "string", "format": "ipv4"}, "."),  # Contains dots
    ]

    for schema, expected_substring in test_cases:
        result = generate_example_from_schema(schema)
        assert expected_substring in result, f"Format {schema.get('format')} should contain '{expected_substring}'"
