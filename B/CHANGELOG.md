...
### Added
- Enhanced tool description formatting with clear parameter documentation
  - New bold **Summary** line at the top with parameter count hint (e.g., "(2 required, 1 optional params)")
  - New `### Parameters:` section with **Path Parameters**, **Query Parameters**, and **Body Parameters** subsections
  - Each parameter displays: name (in backticks), type, `*required*` marker if required, default value, and description
  - Long descriptions are automatically truncated in the summary section for readability
- Improved example response generation with contextual placeholder values
  - Property names like `id`, `name`, `email`, `price`, etc. now generate realistic examples
  - Support for more string formats: `uuid`, `time`, `hostname`, `ipv4`, `ipv6`
  - Enum values are used when available in schemas
  - Integers and numbers generate contextual values (e.g., `price` -> `99.99`, `page` -> `1`)
- Auto-generated default descriptions for parameters without explicit descriptions
  - Common patterns recognized: IDs (`user_id` -> "Unique identifier for the user"), pagination (`limit`, `offset`, `page`), sorting/filtering (`sort_by`, `search`, `q`), boolean flags (`is_active`, `include_details`)
  - Path parameters without descriptions get contextual defaults (e.g., "The resource_name to operate on")

### Changed
- Tool descriptions now follow a clearer structure: Summary -> Parameters (by type) -> Responses
- Parameters are grouped by source (path, query, body) with clear section headers
- Required parameters always appear before optional parameters within each group
- Parameter information is more visually distinct with inline code formatting for parameter names
- Tool description format is now consistent across all endpoints
