...
def generate_default_param_description(param_name: str, param_type: str, param_source: str = "") -> str:
    """
    ...
    Args:
        ...
        param_source: The source of the parameter (path, query, body)

    Returns:
        A generated description string, or empty string if no good default can be determined
    """
    name_lower = param_name.lower()

    # Common ID patterns
    if name_lower == "id" or name_lower.endswith("_id") or name_lower.endswith("id"):
        entity = name_lower.replace("_id", "").replace("id", "").strip("_")
        if entity:
    ...
    if name_lower in ("page", "page_number", "pagenumber"):
        return "Page number for pagination (1-indexed)"
    ...
    if name_lower in ("tags", "labels"):
        return "List of tags or labels"
    if name_lower in ("enabled", "active", "is_active"):
        return "Whether this is enabled/active"
    if name_lower in ("include_details", "detailed", "verbose"):
        return "Include additional details in the response"

    # Date/time fields
    if "date" in name_lower or "time" in name_lower:
        if "start" in name_lower or "from" in name_lower or "begin" in name_lower:
            return "Start date/time for filtering"
        if "end" in name_lower or "to" in name_lower or "until" in name_lower:
            return "End date/time for filtering"
    ...
    # Numeric ranges
    if name_lower.startswith("min_") or name_lower.startswith("minimum_"):
        field = name_lower.replace("min_", "").replace("minimum_", "")
        return f"Minimum {field.replace('_', ' ')} value"
    if name_lower.startswith("max_") or name_lower.startswith("maximum_"):
        field = name_lower.replace("max_", "").replace("maximum_", "")
        return f"Maximum {field.replace('_', ' ')} value"

    # Boolean flags with common prefixes
    if name_lower.startswith("is_") or name_lower.startswith("has_") or name_lower.startswith("can_"):
        flag = name_lower.replace("is_", "").replace("has_", "").replace("can_", "")
        return f"Whether {flag.replace('_', ' ')}"
    if name_lower.startswith("include_"):
        field = name_lower.replace("include_", "")
        return f"Whether to include {field.replace('_', ' ')}"
    if name_lower.startswith("exclude_"):
        field = name_lower.replace("exclude_", "")
        return f"Whether to exclude {field.replace('_', ' ')}"

    # Path parameters typically identify a resource
    if param_source == "path":
        # Convert snake_case to readable format
        readable_name = param_name.replace("_", " ")
        return f"The {readable_name} to operate on"

    # No good default found
    return ""
...
def _format_param_group(
    ...
    # Determine param_source from group_title
    param_source = ""
    ...
    for param_name, param_info in params:
        ...
        if not param_desc:
            param_desc = generate_default_param_description(param_name, param_type, param_source)
